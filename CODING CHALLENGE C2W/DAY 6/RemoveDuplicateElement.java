/*
 * Code 1: Remove Duplicates from the Sorted Array
 ...
 */
 ...
 public class RemoveDuplicateElement{
    ...
    public int removeDuplicates(int[] nums) {
        ...
        int uniqueCount = 1; // Number of unique elements, initialized with the first element
        int currentPointer = 1; // Pointer to the current element being processed

        for (int i = 1; i < nums.length; i++) {
            if (nums[i] != nums[i - 1]) {
                nums[currentPointer] = nums[i]; // Move the unique element to the current position
                uniqueCount++;
                currentPointer++;
            }
        }
